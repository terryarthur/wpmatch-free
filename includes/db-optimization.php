...
class WPMF_DB_Optimization {
	...
	public static function get_stats() {
		...
		$stats['activity'] = array(
			'messages_week' => (int) $wpdb->get_var( $wpdb->prepare(
				"SELECT COUNT(*) FROM {$prefix}messages WHERE created_at >= %s",
				$week_ago
			) ),
			'likes_week' => (int) $wpdb->get_var( $wpdb->prepare(
				"SELECT COUNT(*) FROM {$prefix}likes WHERE created_at >= %s",
				$week_ago
			) ),
			'active_users_week' => (int) $wpdb->get_var( $wpdb->prepare(
	...
	if ( isset( $_GET['action'] ) ) {
		switch ( $_GET['action'] ) {
			...
			case 'rebuild_indexes':
				...
				$success_count = array_sum( $results );
				$total_count   = count( $results );
				add_action( 'admin_notices', function() use ( $success_count, $total_count ) {
